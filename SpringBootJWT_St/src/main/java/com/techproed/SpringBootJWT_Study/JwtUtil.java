...
public class JwtUtil {//bir depo gibi sık kullanılan methodların saklandığı class'tır.İnterview de mutlaka project anlatılırken sorulur
    ...
    public Date extractExpiration(String token) {
        return extractClaim(token, Claims::getExpiration);//token'in ne zaman expire olacagını return eder.Son kullanma
    }
    ...
    public Boolean validateToken(String token, UserDetails userDetails){//App create ettiği token ile client'den gelen token  oney controlu
        ...
        final String username= extractUsername(token);//token extractUserName() method call ederek return olan username'i string type'da veriable ata
